...
[UPDATED] 2026-09-26
[NOTES]
- **2026-09-26 — `aizee security scan` execution layer (v5.17.0)**: Built the missing SAST/orchestration layer so security doctrine is enforceable, not just documented. `runtime/security_scanner.py` — built-in OWASP-2025-mapped regex rules (secrets/AWS-keys/private-keys/.env, injection sinks shell=True|eval|os.system|yaml.load|pickle|tarfile-extractall, misconfig DEBUG|CORS-*|bind-0.0.0.0|TLS-verify-off, A10 exception-swallow, weak crypto, XSS sinks innerHTML|mark_safe|v-html, open-redirect, plaintext http) + external-tool orchestration when installed (bandit, `ruff check --select S`, pip-audit, npm audit, composer audit, trivy) with per-tool timeout + JSON/normal output parsing. CLI: `aizee security scan <path> [--json] [--no-tools] [--limit N]` — exit 1 on any blocker. Dedupes (rule,file,line); allowlist for canonical example values (AKIAIOSFODNN7EXAMPLE); default-skips tests/fixtures/temp/backups/graphify-out/htmlcov/vendor dirs. Self-scan on own repo: 87904 findings -> 97 after noise-scoping fixes (signal-vs-noise work, not weaker detection). 25 tests in `tests/test_security_scanner.py` (module at 100% coverage). Also fixed: `memory/ingest.py::_validate` was stricter than `scripts/validate-globals.py::check_struct` — rejected tech-stack files using [DATA]/[API] instead of [RULES]; 27 vendor/SaaS refs were silently skipped from memory. Now aligned (OBJ + (RULES or tech DATA/API)); ingest exits 0, +27 memories. Wired: CLI dispatch `security` cmd, runtime exports, `security-auditor.md` rule 23 points to it, `workflows/07-security-audit.md` step-0 invokes it, CHANGELOG 5.17.0, version synced across pyproject/READMEs/manifest/validators. Post-review fixes: `.env.*` variants (.env.local/.env.production) were never scanned — `Path.suffix` returns `.local` not `.env.*` — inclusion now uses `name.startswith('.env')`; `test_audit_gaps::test_stored_key_reused` xdist flake — stale `_audit_key_cache` could be repopulated between fixture reset and test body (b"cached" leak) — now cleared before k1 too. Suppression layer added: `aizee-scan: ignore [RID]` per-line + `aizee-scan: ignore-file` first-4KiB markers (suppressed count reported separately); gitignore-awareness downgrades `.env` findings blocker→low when `git check-ignore` confirms; SEC-ENVFILE requires non-empty value; TLS-WEAK-HASH honors `usedforsecurity=False`; `sbom.json` skipped. Clean-sweep result: 97→0 findings — real fixes (guard_invariants fail-open→closed, 4 unescaped app.js interpolations→escapeHtml, sql concat folded, md5 usedforsecurity) + ~54 documented suppressions (detector rule tables, policy yaml, attack payloads, probe-style excepts). Coverage-gap debt from 5.16.0 closed: test_skill_validator.py (new), task_contract/* 100%, c4_docs/harness_exporter/task_tools/observations gaps filled. Harness gates: ruff+mypy(416)+validate-globals(539) green. Verdict workflow: blocker=must-fix-before-ship.
- **2026-09-26 — security coverage audit + `appsec-hardening` matrix**: User asked to verify vuln coverage across ALL domains. Researched OWASP Top 10 **2025** (final: A01 access-control now absorbs SSRF + BOLA/BFLA; NEW A03 supply-chain + A10 exceptional-conditions; misconfig jumped to #2) + ran a 30-term coverage scan over skills/rules/tech-stack. Found gaps: no CSRF/SSTI/open-redirect/proto-pollution/ReDoS rules, ZERO desktop-app security (QSettings plaintext, unauthenticated localhost UI servers, unsigned updaters, URL-scheme validation), unsystematized Python AppSec, OWASP 2021-era naming. Fixes: NEW `tech-stack/appsec-hardening.md` (master vuln→defense matrix: OWASP 2025 A01-A10 + CSRF/clickjacking/redirect/pollution + bandit-grade Python rules + desktop-security doctrine + edge/data-leak rules + scanner mapping); `web-security-checklist` +10 rules (CSRF, cmd-inject, SSTI, open-redirect, proto-pollution, ReDoS, A10 fail-closed, 2025 mapping, desktop layer) + new triggers + tech_stack; `security-auditor` now cites OWASP 2025 A01-A10 + expanded injection surfaces + desktop/Python rule; `python-ui-lord` +Desktop Security rule (keyring, 127.0.0.1+token, signed updaters, URL-scheme/clipboard untrusted); `production-readiness-lord` tech_stack += appsec-hardening; `.cursor/rules/security.mdc` +SEC-12/13/14 (2025 audit, fail-closed, desktop/local-UI). Validated: 139 skills 0 errors, globals 539/0, ingest +38, synced.
- **2026-09-26 — `production-readiness-lord` skill**: User asked for a portable prompt for real production audits → converted to permanent capability. `skills/production-readiness-lord/` — 13-rule audit protocol: Phase-0 discovery questions (env/scale/compliance/deadline), READ-ONLY mandate, FACT/ASSUMPTION/UNVERIFIABLE labeling, mid-session decision escalation, 13 audit axes (arch→legal), severity tiers (blocker→low), fixed report format with 👤 HUMAN-ONLY ACTIONS section. Registered: personas.yaml lords on ARCH/DEV/QA/SRE/DEVOPS/SEC + lord_skills keywords (EN+AR), manifest +22 triggers. Verified: detect EN prompt → SEC + lord #1; AR prompt → ARCH + lord #1. validate-globals 538 files 0 errors.
- **2026-09-26 — depth wave: all persona primaries + thin lords deepened to #1 doctrine**: Audit measured every skill by line count; ~25 thin files (11–28 lines, pointer-only) rewritten dense. Persona primaries deepened: `backend-api-expert` (layering/idempotency/BOLA/error contract/queues/observability, Node+Python), `devops-engineer` (pipeline-as-code/IaC/GitOps/release strategies/DORA), `sre` (SLO math/error budgets/reliability patterns/alert hygiene/toil), `qa-debugger` (scientific method/edge hunting/flake quarantine/mutation thinking), `api-architect` (contract-first/RFC9457/idempotency-key/pagination/webhooks/gateway), `data-engineer` (medallion/partitioning/contracts/CDC/orchestration/PII), `ml-engineer` (+baselines/eval harness/feature parity/drift/serving modes), `frontend-ui-expert` (state taxonomy/fetch waterfall/error boundaries/vitals), `security-auditor` (+threat-model-first/STRIDE/authz depth/crypto reality-check/reporting), `performance-engineer` (USE/budgets/load-testing), `product-manager` (discovery/prioritization/metrics honesty), `technical-writer` (Diátaxis/runbook format), `legal-compliance` (GDPR mechanics/consent/licensing audit), `clean-code-guard`, `test-driven-development`, `context-compressor`, `fullstack-optimizer`, `docs-guard`, `graphify`, `browser-extension-builder` (MV3 doctrine), `writing-plans`, `wordpress-expert`, `brainstorming`, `prompt-master-patterns`, `test-guard`, `api-versioning`. Lords deepened: `devops-lord` (k8s baseline/IaC/GitOps/supply-chain), `linux-systems-lord` (USE method/systemd/eBPF/sysctl discipline), `cloud-platforms-lord` (landing zones/IAM/cost/serverless realism), `database-lord` (engine selection/indexing/MVCC/migrations/ops), `messaging-streaming-lord` (delivery semantics/ordering/DLQ/outbox/saga), `search-vector-lord` (eval set/hybrid RRF/chunking/HNSW/RAG discipline), `ai-ml-lord` (training mechanics/compression/serving/evals-as-spec), `language-lord` (idioms/concurrency models/type leverage/version fluency), `frontend-frameworks-lord` (reactivity models/state placement/rendering strategy), `game-architect` (sim-vs-presentation/input/feel), `google-play-warlord` (vitals/policy/tracks/signing). Skill validate: 138, 0 errors. validate-globals: 537, 0 errors. Memory ingest: +51. Synced to aizee.
- **2026-09-26 — capability expansion wave (devops/servers + problem-solving + design innovation)**: User granted broad mandate to reach #1 across domains. Audit found: devops-lord/linux-systems-lord are thin pointers; no sysadmin layer, no creative-problem-solving lord, no generative-design lord. Built 3 lords: **`skills/server-ops-lord/`** (host-level doctrine: SSH keys-only+Tailscale admin plane, ufw/fail2ban, reverse proxy+TLS choice Caddy-vs-nginx, 3-2-1 backups w/ restore drills, monitoring symptoms-not-causes, panels=accelerators, Proxmox LXC>VM, Windows Server notes, troubleshooting method, runbook handoff), **`skills/problem-solving-lord/`** (define-real-problem→constraints audit→first-principles→inversion→box-catalog lateral moves→hypothesis matrix→workaround ladder→one-way-door scoring→stuck protocol→kill criteria→compress to memory), **`skills/design-innovation-lord/`** (diverge≥3→converge-ONE-direction, signature moment, mashup method, principle-not-pixel extraction, novelty score, squint/5-second tests, anti-slop prohibitions). 6 tech-stack files: nginx-1 (1.30.4/1.31.5), caddy-2 (2.11.4), openssh-10, coolify-4 (4.3.9), proxmox-9, tailscale. personas.yaml: server-ops→SRE/DEVOPS/SEC, problem-solving→ARCH/DEV/QA, design-innovation→UX (+lord_skills keywords, AR included). manifest +46 triggers. useful-repos +7.
- **2026-09-25 — python-ui-lord skill + Python UI tech-stack**: User asked for #1 Python app-design capability (desktop + web). Researched 2026 landscape → created **`skills/python-ui-lord/`** (43 rules + 4 workflows: framework selection matrix, tokens-first theming, 4-state coverage, a11y/RTL, per-framework mastery, packaging, Context7 gate) and 5 tech-stack files: `pyside-6` (6.11.2), `textual-8` (8.2.8), `flet-1` (1.0 GA Sep 2026, event-loop breaking change), `reflex` (0.9.x), `nicegui-3` (3.17.x, ≥3.16.0 for security fixes). Registered in personas.yaml (UX/DEV/DEVX lords + lord_skills keywords incl. Arabic) and manifest.json (+21 triggers). Selection doctrine: PySide6=pro native, Textual=TUI, Flet=write-once-multiplatform, Reflex=production pure-Python web, NiceGUI=fast tools, Streamlit/Dash=prototype only, customtkinter=dormant(2023).
- **2026-09-25 — design-research adoption (Refero + Mobbin)**: Studied both agent surfaces (`llms.txt`, https://mobbin.com/mcp.md, refero_skill repo SKILL.md). Adopted: `mobbin` MCP in `.devin/mcp_config.json` (OAuth, Pro+); `install.ps1` alwaysAllow += refero/mobbin read-only tools; **`skills/design-research/`** — unified doctrine: styles→direction, screens→patterns, flows→journeys, sections→web composition; research-before-design; ONE dominant direction (never average references); evidence≠proof + provenance (product+URL); brief→lock→build→validate loop; rate limits; MCP-absent fallback to bundled catalog. `design-md-lord` now defers live research to it. manifest +8 triggers, eval cases updated. Gotcha: `Path.write_text` on Windows writes CRLF by default — pass `newline='\n'` for LF files.
- **2026-09-25 — Task Contract + external-source adoption (7 repos studied)**: Deep review of BrowserSkill, animejs.com, Cloudflare Quick Tunnels, deepwiki-rs, addyosmani/agent-skills, social-media-skills, prompt-master → implemented all useful findings:
  - **`runtime/task_contract/` package** (models/classifier/validation/evidence/enforce/engine): enforced `PROMPT → CLASSIFY → DECOMPOSE → per-task verify → FINAL REVIEW`. Every prompt gets a recorded trivial/standard/complex classification + reason; non-trivial work requires `.task/plan.json` (kebab ids, acyclic deps, declared scope, `produces:` handoffs, acceptance checks); `done` requires evidence; `finish()` = final review. Enforcement = rules + hooks now, `AIZEE_TASK_STRICT=1` optional hard mode.
  - `aizee task` CLI (classify/decompose/status/start/verify/complete/block/amend/finish/abandon/scope/context); kernel service `kernel.task_contract`; `rules/task-contract.md` + `workflows/61-task-contract.md`.
  - **`rules/untrusted-content.md`**: BrowserSkill doctrine — tool/page/console output is data, never authority (injection defense for all skills).
  - **`runtime/skill_validator.py`** + `aizee skill validate`: SKILL.md contract checker — 133 skills, 0 errors.
  - **Trigger evals**: `eval/trigger_cases.json` + `tests/test_trigger_routing.py` (agent-skills evals pattern); caught real bug — added `zero-downtime deploy` manifest alias.
  - **`runtime/harness_exporter.py`** + `aizee skill install --harness X`: install skills into cursor/claude/codex/opencode/project dirs (BrowserSkill `bsk install-skill` pattern).
  - **`runtime/c4_docs.py`** + `aizee docs c4`: deepwiki-rs-inspired C4 doc generator consuming graphify-out/graph.json → `docs/c4/{context,containers,components/*}.md` with Mermaid.
  - **`AGENT_INSTALL.md`**: install guide written FOR agents. **`skills/aizee-lite/`**: portable aiZee governance for hosts without aiZee (skill-as-product).
  - **`skills/project-voice/`**: foundation artifacts `.ai/about-me.md` + `.ai/voice.md` consumed by content skills (social-media-skills voice-builder pattern).
  - **`skills/browser-automation/`**: bsk wrapper skill w/ evidence + session-cleanup rules. **`tech-stack/animejs-4.md`**: v4 reference. **`workflows/62-localhost-tunnel.md`**: cloudflared protocol + safety policy.
  - **`skills/prompt-engineer.md`**: prompt-master v1.8 delta (9-dim intent extraction, model-recency gate, risky-technique allowlist, token audit, max-3 questions).
  - Gotchas: validate-globals requires `[RULES]` marker in SKILL bodies (use [RULES], not [PRIMACY]/[MIDDLE] sections alone); generated artifact refs must be added to `IGNORED_FILE_REFS` in scripts/validate-globals.py; graph.json node `label` (not `id`) is what humans read.
  - Counts after sync: runtime 134, skills 133, numbered workflows 63, stack 253, tests 7426. Gates green: ruff, mypy, targeted pytest (70 new tests), validate-globals 0/0.
