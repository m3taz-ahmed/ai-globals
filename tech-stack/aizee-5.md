[TECH] aizee-5
[OBJ] aiZee v5.5.0 — sovereign AI engineering control plane. Internal standards for self-reference (dogfooding).
[RULES]
...
- Runtime: 88 governance modules in `runtime/` (commands, scoped_manager, hook_lifecycle, layers, contract_emitter, middleware, checkpoint, schema_contract).
- MCP: `aizee_mcp/` (35 tools via FastMCP) + `aizee_mcp/tools/` (5 tool modules).
- Memory: `memory/` (SQLite + FTS5 + vector).
- Skills: 66 persona + lord skills in `skills/`.
- Workflows: 30 trigger-based protocols in `workflows/`.
- Tech-stack: 162 version-locked stack references in `tech-stack/`.
[COMPAT]
