...
9. [REQ] Laravel Architecture Patterns (ranked by complexity):
   1. Simple: Controller + Model + FormRequest (Laravel default â€” small apps)
...
10. [REQ] Pattern Selection Matrix (choose based on project type):
   - eCommerce / Multi-vendor â†’ Modular (Concord) like Bagisto (41 packages, EAV, 22 locales)
   - CRM / SaaS â†’ DDD + Service Layer like Monica (Accountâ†’Vault hierarchy, WebAuthn)
   - API-first / Headless â†’ Repository + DTO + API Resources + Versioning like Koel (header-based versioning, cursor pagination)
   - Wiki / Content platform â†’ Feature-based + Activity Logging like BookStack (jhfa content filtering, MFA)
   - Admin Panel â†’ Filament + Plugin System + Clusters (see `filament-plugins` tech-stack)
...
13. [REQ] DTO Rules (Koel pattern): `app/Values/` folder for value objects. FormRequest with `toDto(): SomeData` for conversion. Named arguments in DTO constructors. `SongUpdateData::make(title:, artistName:, ...)`. â›” NEVER pass raw `array $data` between Service layers.
14. [REQ] API Design Rules (Koel pattern): `JsonResource` with `JSON_STRUCTURE` + `PAGINATION_JSON_STRUCTURE` + `CURSOR_PAGINATION_JSON_STRUCTURE` constants. Header-based versioning: `X-Api-Version` â†’ `routes/api.{version}.php`. `RouteServiceProvider::loadVersionAwareRoutes(string $type)`. Cursor pagination for large datasets (>10k rows), offset for standard. `PaginationStrategyResolver` selects based on request.
15. [REQ] Multi-Tenancy Rules (Monica/Bagisto pattern): Hierarchical: Account â†’ Vault with VIEW(300)/EDIT(200)/MANAGE(100) permissions + pivot table. Channel-based: each Channel has currency/locale/theme separate. â›” NEVER trust tenant isolation without global scope. â›” NEVER expose one tenant's data to another. Query `laravel-security` tech-stack for full rules.
...
18. [REQ] AI Integration (Bagisto/Krayin pattern): Laravel AI SDK (`laravel/ai`) for native `whereVectorSimilarTo()` + `->stream()`. Custom `MagicAIService` with system prompt templates + recursive call prevention (`static $isExtracting`). Treat LLM outputs as untrusted â€” validate schema. Query `laravel-ai` tech-stack.
