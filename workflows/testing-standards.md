...
4. [REQ] Coverage `[TEST-06]`: 80% logic, 90% APIs. NO skipping tests without linked issue.
...
6. [REQ] `[TEST-08]` Per-stack fast/full commands (see `workflows/testing-tiers.md`):
   ...
   - Python/pytest: `pytest <file> -q --no-cov` (fast) / `pytest --cov --cov-fail-under=80` (full)
