...
class UninstallerGUI:
    ...
    def _build_ui(self) -> None:
        ...
        self.tree.bind("<Return>", self._on_toggle_row)

        # --- Action buttons row ---
        btn_frame = ttk.Frame(self.win)
        btn_frame.pack(fill="x", padx=16, pady=4)

        ttk.Button(btn_frame, text="Keep All Learned", command=self._keep_all_learned).pack(
            side="left", padx=2
        )
        ttk.Button(btn_frame, text="Delete All", command=self._delete_all).pack(
            side="left", padx=2
        )
        ttk.Button(btn_frame, text="Toggle Selected", command=self._on_toggle_row).pack(
            side="left", padx=2
        )

        # --- Backup row ---
        backup_frame = ttk.Frame(self.win)
        ...
        self.backup_path_var = tk.StringVar(
            value=str(Path.home() / f"aizee-backup-{datetime.now().strftime('%Y%m%d-%H%M%S')}.zip")
        )
        self.backup_entry = ttk.Entry(backup_frame, textvariable=self.backup_path_var, width=45)
        self.backup_entry.pack(side="left", padx=4)
        self.browse_btn = ttk.Button(backup_frame, text="Browse...", command=self._browse_backup)
        self.browse_btn.pack(side="left", padx=2)

        # --- Log area ---
        log_frame = ttk.LabelFrame(self.win, text="Log", padding=4)
        ...
        self.uninstall_btn = ttk.Button(
            bottom, text="Uninstall", command=self._on_uninstall_click
        )
        self.uninstall_btn.pack(side="right", padx=4)

        ttk.Button(bottom, text="Cancel", command=self.win.destroy).pack(
            side="right", padx=4
        )
